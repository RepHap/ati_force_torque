...
__gc class ATIDAQHardwareInterface
{
public:
	...
	*/
	int32 ReadSingleSample( float64  buffer __gc[] );
	...
	*/
	int32 ReadBufferedSamples( int numSamples, float64 buffer __gc[] );
...
private:
	TaskHandle __nogc*	m_thDAQTask;		/*the task which is used to get a data*/	
	...
	String *			m_sDeviceName ;		/*the name of the NI-DAQmx device which the transducer is attached to*/
	...
	String *			m_sErrorInfo;		/*information about the last error*/
	...
	*/
	char __gc* ati_strncpy( char __gc* dest, char const __gc* source, unsigned int len );
	...
	returns:
		pointer to dest
	*/
	char __gc* ati_strncat( char * dest, char const __gc* catsource, unsigned int len );
	...
	*/	
	unsigned long ati_nstrlen( char const __gc* buffer, unsigned long buffersize );
	 ...
	 */
	int32 ati_Channel_Name_Format( char * channelString, 
