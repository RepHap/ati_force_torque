...
int ATICombinedDAQFT::FTSystem::LoadCalibrationFile( String * calFile, int calibrationIndex )
{
	...
	for ( i = 0; (i < calFile->get_Length()) && ( i < ( WHOLE_LOTTA_CHARACTERS - 1 ) ) ; i++ )
	{
		cStringCalFile[i] = (char)calFile->get_Chars(i);
	}	
	cStringCalFile[ calFile->get_Length() ] = '\0';
	m_Calibration = createCalibration( cStringCalFile, calibrationIndex );
	if ( NULL == m_Calibration )
	{
		m_sErrorInfo = new String( "Could not load calibration file successfully" );
		return -1;
	}	
...
int ATICombinedDAQFT::FTSystem::ReadSingleGaugePoint( float64 gaugeValues __gc[] )
{
...
String * ATICombinedDAQFT::FTSystem::GetSerialNumber()
{
	...
	return new String( m_Calibration->Serial );
}
...
String * ATICombinedDAQFT::FTSystem::GetCalibrationDate()
{
	...
	return new String( m_Calibration->CalDate );
}
...
String * ATICombinedDAQFT::FTSystem::GetForceUnits()
{
	if ( NULL == m_Calibration )
		return new String( "" );
	return new String( m_Calibration->cfg.ForceUnits );
}
...
String * ATICombinedDAQFT::FTSystem::GetTorqueUnits( )
{
	if ( NULL == m_Calibration )
		return new String( "" );
	return new String( m_Calibration->cfg.TorqueUnits );
}
...
int ATICombinedDAQFT::FTSystem::ToolTransform( float64 transformVector __gc[], String * distanceUnits, String * angleUnits )
{
	float tempTransforms __nogc[6]; /*unmanaged array of transformation values*/
...
int ATICombinedDAQFT::FTSystem::GetTransformVector( float64 transformVector __gc[] )
{
	if ( NULL == m_Calibration )
...
String * ATICombinedDAQFT::FTSystem::GetTransformDistanceUnits()
{
	if ( NULL == m_Calibration ) 
		return new String( "" );
	return new String( m_Calibration->cfg.UserTransform.DistUnits );
}
...
String * ATICombinedDAQFT::FTSystem::GetTransformAngleUnits()
{
	if ( NULL == m_Calibration )
		return new String( "" );
	return new String( m_Calibration->cfg.UserTransform.AngleUnits );
}
...
String * ATICombinedDAQFT::FTSystem::GetBodyStyle()
{
	if ( NULL == m_Calibration )
		return new String( "" );
	return new String( m_Calibration->BodyStyle );
}
...
String * ATICombinedDAQFT::FTSystem::GetCalibrationType()
{
	if ( NULL == m_Calibration )
		return new String( "" );
	return new String( m_Calibration->PartNumber );
}

int ATICombinedDAQFT::FTSystem::GetWorkingMatrix( 
							float64 matrix __gc[ati_ARRAY_DEC2(6,6)] )
{
	if ( NULL == m_Calibration )
		return 1;
...
int ATICombinedDAQFT::FTSystem::ReadSingleFTRecord( double readings __gc[] )
{
	if ( NULL == m_Calibration )
		return 1;
	int retVal = 0; /*the return value*/
	float64 ati_ARRAY_DEF(gcVoltages) = new float64 __gc[ NUM_STRAIN_GAUGES + 1]; /*allow an extra reading for the thermistor*/	
	float nogcVoltages __nogc[NUM_STRAIN_GAUGES + 1]; /*non-gc version for passing to c library*/
	int i; /*generic loop/array index*/
	float tempResult __nogc[NUM_FT_AXES]; /*copy of readings read using C library*/
...
int ATICombinedDAQFT::FTSystem::BiasCurrentLoad()
{
	if ( NULL == m_Calibration )
		return 1; /*calibration not initialized*/
	float64 ati_ARRAY_DEF(curVoltages) = new float64 __gc[NUM_STRAIN_GAUGES + 1]; /*the current strain gauge load*/
...
int ATICombinedDAQFT::FTSystem::BiasKnownLoad( float64 biasVoltages __gc[] )
{
	if ( NULL == m_Calibration )
		return 1;
	float nogcVoltages __nogc[NUM_STRAIN_GAUGES]; /*version of bias voltages which can be passed to the c library*/
	int i; /*generic loop/array index*/
	/*
...
{
	int i; /*generic loop/array index*/
	for ( i = 0; ( i < destSize ) && ( i < sourceString->get_Length() ); i++ )
	{
		destString[i] = (char)sourceString->get_Chars(i);
	}
	destString[i] = '\0';
}
...
int ATICombinedDAQFT::FTSystem::ReadBufferedGaugeRecords( int numRecords, double readings __gc[] )
{
	...
	if ( CheckForGaugeSaturation( numRecords, readings ) ) status = 2;
	return status;
}

bool ATICombinedDAQFT::FTSystem::CheckForGaugeSaturation( int numRecords, double readings __gc[] )
{
	...
	for( i = 0; i < numRecords * m_hiHardware->GetNumChannels(); i++ )
	{
...
int ATICombinedDAQFT::FTSystem::ReadBufferedFTRecords( int numRecords, double readings __gc[] )
{
	...
	float64 ati_ARRAY_DEF(gaugeValues) = new float64 __gc[numGaugeValues]; 	
		/*the gauge readings which are fed to the c library*/
	float currentGaugeReadings __nogc[ NUM_STRAIN_GAUGES + 1 ]; /*current gauge reading*/
	float currentFTValues __nogc[ NUM_FT_AXES ]; /*current f/t reading*/
...
int ATICombinedDAQFT::FTSystem::GetBiasVector( double biasVector __gc[] )
{
	if ( NULL == m_Calibration )
		return 1;
	int i; /*generic loop/array index*/
